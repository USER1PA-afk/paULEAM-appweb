...
| **POS sale fails with "No token provided"** | `process_kiosk_sale` is SECURITY DEFINER — must be called via the server proxy `/api/pos/sale`, not directly from the browser SDK. The SDK's `edgeFunctionToken` is not recognized by PostgREST as a user context. See `docs/POS-TROUBLESHOOTING.md` for the full runbook. |
| **POS products grid empty / 400 on `stock_summary`** | The view is missing a column the POS query requests. `features/pos/hooks/index.ts:loadPosProducts` has a 3-tier fallback; while the migration is pending, products load via the `products` table and show "Agotado". Apply the matching migration to clear the warnings. See `docs/POS-TROUBLESHOOTING.md`. |
| **Infinite 403 loop on `/api/auth/refresh`** | Stale `refresh_token` in localStorage. `shared/components/auth-recovery-boot.tsx` auto-clears + reloads on the first 401/403. If it doesn't fire, hard-reload with DevTools open and run `localStorage.clear()` manually. See `docs/POS-TROUBLESHOOTING.md`. |
| **PWA install banner never appears** | The `beforeinstallprompt` listener must be mounted globally. `app/layout.tsx` mounts `<InstallPwaButton />` in the root layout. Also requires `app/manifest.ts` (or `public/manifest.webmanifest`, NOT both) + a registered `/sw.js` with a `fetch` handler. |
...
- **Project:** **ProyectoBackendPA** (API base `https://8i4ga35v.us-east.insforge.app`)
