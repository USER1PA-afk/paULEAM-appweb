-- ============================================================
-- PAuleam ERP — Fix: Corregir especificadores de formato en format()
-- ============================================================
-- PROBLEMA: En la función execute_unified_production, dos llamadas
-- a format() usan "%" en lugar de "%s" para inyectar el nombre del producto,
-- lo que causa el error "unrecognized format() type specifier".
--
-- SOLUCIÓN: Reemplazar "%" por "%s" en:
--   1. Mensaje de egreso de material de empaque.
--   2. Mensaje de ingreso de producto terminado.
-- ============================================================
  ...
  LOOP
    ...
    IF FOUND THEN
      ...
      LOOP
        ...
        -- EGRESO del material primario (Corregido "%" -> "%s")
        INSERT INTO public.inventory_ledger (
          product_id, movement_type, quantity, unit_cost,
          reference_type, reference_id, notes, created_by
        ) VALUES (
          ...
          'PRODUCCION_UNIFICADA',
          p_order_id,
          FORMAT('Lote %s — Material empaque para "%s": %s × %s %s',
            v_batch_label,
    ...
    -- INGRESO del producto terminado (Corregido "%" -> "%s")
    INSERT INTO public.inventory_ledger (
      product_id, movement_type, quantity, unit_cost,
      reference_type, reference_id, notes, created_by
    ) VALUES (
      ...
      'PRODUCCION_UNIFICADA',
      p_order_id,
      FORMAT('Lote %s — "%s": %s unidades × %s kg — Costo/u: $%s',
        v_batch_label,
        v_presentation.product_name,
        v_presentation.units_to_produce,
