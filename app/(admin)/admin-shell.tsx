...
/**
 * Standalone nav link — declared at module scope to keep a stable identity
 * across renders (required by React 19's static-components rule).
 */
function NavLink({
  item,
  pathname,
...
/**
 * Client shell rendered inside the (admin) server layout. Owns the auth/role
 * redirect logic, the sidebar, the header and the notifications panel.
 */
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  // Keep the store submenu expanded when navigating to /admin/store/* or
  // /shop/catalog. The initializer handles first paint; this effect keeps
  // the open state in sync on subsequent client-side navigations.
  useEffect(() => {
          ...
          >
            ...
            <div>
              ...
              <ul className="space-y-0.5">
                ...
                {role === "operario" && (
                  <NavLink
                    item={{ label: "Ver E-Commerce", href: "/shop/catalog", icon: Store, roles: ["admin", "operario"] }}
                    pathname={pathname}
                    sidebarCollapsed={sidebarCollapsed}
                    onNavigate={() => setSidebarOpen(false)}
                  />
